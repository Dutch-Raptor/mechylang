    ...
    fn as_pretty_error_builder<'a>(&self, source_id: &'a str) -> ReportBuilder<(&'a str, Range<usize>)> {
        ...
        match self {
            ...
            Error::UnsupportedInfixOperator {
                ...
                Report::build(ReportKind::Error, source_id, left_span.start())
            ...
            Error::TypeError { span, expected, found, context } => {
                ...
                let expected = if expected.len() == 1 {
                    ...
                    format!("one of {}", expected.iter().map(ToString::to_string).join(", ").fg(expected_type_color))
                };
                    ...
                    .with_label(
                        Label::new((source_id, span.bytes.clone()))
                            .with_message(format!("this has type `{type}`",
                                                  type = found.fg(found_type_color))).with_color(found_type_color)
                            .with_priority(1))
                    .with_label(
                        Label::new((source_id, span.bytes.clone())).with_message(
                            format!("but expected type {expected}", expected = &expected))
                            .with_color(expected_type_color))
                    .tap_owned(|mut report| {
                        if let Some(context) = context {
                            report = report.with_label(
                                Label::new((source_id, context.bytes.clone())).with_message(
                                    "in this context".to_string())
                                    .with_priority(0)
                                    .with_order(1000)
                            );
                        }
            ...
            Error::AssertionEqualFailed { first_span, second_span, first_value, second_value, assert_span } => {
                let first_value_color = color_generator.next();
                let second_value_color = color_generator.next();
                Report::build(ReportKind::Error, source_id, first_span.start())
                    .with_message("Assertion failed".to_string())
                    .with_label(
                        Label::new((source_id, first_span.bytes.clone()))
                            .with_message(format!("this value `{value}`",
                                                  value = first_value.fg(first_value_color))).with_color(first_value_color)
                            .with_order(0)
                            .with_priority(1))
                    .with_label(
                        Label::new((source_id, second_span.bytes.clone()))
                            .with_message(format!("is not equal to `{value}`",
                                                  value = second_value.fg(second_value_color))).with_color(second_value_color)
                            .with_order(1)
                            .with_priority(1))
                    .with_label(
            ...
            Error::InvalidPrefixOperatorForType { span, operator, right, operator_span, right_span } => {
                let type1_color = color_generator.next();
                let operator_color = color_generator.next();

                Report::build(ReportKind::Error, source_id, span.start())
                    .with_message(format!("Operator `{operator}` is not defined for {type_name}",
                                          operator = operator,
                                          type_name = right.get_type(),
                    ))
                    .with_label(
                        Label::new((source_id, operator_span.bytes.clone()))
                            .with_message(format!("`{operator}` cannot be applied to a {type_name}",
                    ...
                    .with_label(
                        Label::new((source_id, right_span.bytes.clone()))
                            .with_message(format!("This has type `{type}`",
                                                  type = right.get_type().fg(type1_color))).with_color(type1_color)
                            .with_priority(0)
                            .with_order(1))
            }

            Error::IdentifierNotFound { span, identifier, similar } => {
                Report::build(ReportKind::Error, source_id, span.start())
                    .with_message(format!("Undefined identifier `{identifier}`",
                                          identifier = identifier))
                    .with_label(
                        Label::new((source_id, span.bytes.clone()))
                            .with_message(format!("`{identifier}` is not defined in this scope", identifier = identifier.fg(Color::Red)))
                    ...
                    .tap_owned(|report|
                        if let Some(sim) = similar {
                            report.with_help(
                                format!("A variable with a similar name exists: `{sim}`", sim = sim.fg(color_generator.next()))
                            )
                        } else {
                            report
                        })
            ...
            Error::WrongNumberOfArguments { span, unexpected_arg, expected, found } => {
                ...
                let found_plural_was = if *found != 1 {
                    "were"
                } else {
                    "was"
                };
            ...
            Error::CannotCall { function, span } => {
                ...
                Report::build(ReportKind::Error, source_id, span.start())
                    .with_message(format!("Cannot call `{function}`",
                                          function = function.fg(function_color)))
                    .with_label(
                        Label::new((source_id, span.bytes.clone()))
                            .with_message(
                                format!("`{function}` is not a function",
                                        function = function.fg(function_color))).with_color(function_color)
                            .with_priority(0)
                            .with_order(1))
            }

            Error::IterCallOnNonIterable { obj_span, iter_call_span, reason, obj } => {
                let type_color = color_generator.next();
                Report::build(ReportKind::Error, source_id, iter_call_span.start())
                    .with_message(
                        match reason {
            ...
            Error::IteratorMethodOnIterable { method_span, method_name, object_type, object_span } => {
                ...
                Report::build(ReportKind::Error, source_id, method_span.start())
            ...
            Error::IteratingOverNonIterable { obj_span, reason, obj, for_span } => {
                let type_color = color_generator.next();
                Report::build(ReportKind::Error, source_id, obj_span.start())
                    .with_message(
                        match reason {
                            IntoIteratorError::InvalidRange =>
                                format!("Cannot iterate over `{type}` range",
            ...
            Error::MutateNonExistentVariable { name } => {
                Report::build(ReportKind::Error, source_id, 0)
                    .with_message(format!("Cannot mutate non-existent variable `{name}`"))
            }
            Error::IndexOutOfBounds { array_span, index_span, index, length } => {
                Report::build(ReportKind::Error, source_id, index_span.start())
                    .with_message(format!("Index `{index}` is out of bounds for array of length `{length}`"))
                    .with_label(
            ...
            Error::PropertyNotFound { property, span, object_type } => {
                Report::build(ReportKind::Error, source_id, span.bytes.start)
                    .with_message(format!("Property `{}` not found", property))
                    .with_label(
                        Label::new((source_id, span.bytes.clone()))
                            .with_message(format!("Object of type `{}` has no property `{}`", object_type, property))
                            .with_color(Color::Red)
                            .with_priority(0)
                            .with_order(1))
            }
            ...
            Error::BuiltInError { span, message, error_type } => {
                Report::build(ReportKind::Error, source_id, span.start())
            ...
            Error::AssertionFailed { span, assert_span, value } => {
                Report::build(ReportKind::Error, source_id, span.start())
            ...
            Error::IOError { error, span } => {
                Report::build(ReportKind::Error, source_id, span.clone().unwrap_or_default().start())
            ...
            Error::IndexingNonIndexableType { indexed_span, indexed_obj } => {
                Report::build(ReportKind::Error, source_id, indexed_span.start())
